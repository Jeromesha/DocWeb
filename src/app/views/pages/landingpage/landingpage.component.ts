...
import { InetDataService } from "src/app/services/inetdata.service";
...
export class LandingpageComponent implements OnInit {
  public API_END_POINT_TRANSACTION: any = environment.apiBaseUrl;
  packages: any = [];
  ...
  constructor(
    ...
    private inetDataService: InetDataService,
  ) {
    this.getService()
  }
  ...
  initializeValidators() {
    this.form = this.formBuilder.group({
      ...
      email: ['', [Validators.required, Validators.email]], // Standard email validation
      packageId: ['', [Validators.required]],
      userMessage: ['', Validators.required],
      requestTypeId: [1],
      enquiryStatusTypeId: [1],
      comments: ['']
    });
  ...
  getPackage() {
    this.inetDataService.getPackages().subscribe(res => {
  ...
  getService() {
    var resp = this.inetDataService.getServicePackages().subscribe(res => {
      var res1: any = JSON.stringify(res);
      res1 = JSON.parse(res1);
